function [A, Q, b, c, lb, ub, obj_const_term] = QP_Convert_to_Standard_Form(A, Q, b, c, lb, ub)
% ==================================================================================================================== %
% QP_Convert_to_Standard_Form(A, Q, b, c, lb, ub):
% This function takes as input the data of an LP in the following form:
%                       min    c^T x + (1/2) x^T Q x
%                       s.t.   Ax = b,
%                              lb <= x <= ub
% and transforms it in a standard form, that is:
%                       min    c_bar^T x_bar
%                       s.t.   A_bar x_bar = b
%                              x_bar >= 0.
% 
...
% ____________________________________________________________________________________________________________________ %
    ...
    % ================================================================================================================ %
    % Introduce constraints so every variable has non-negative bounds.
    % ---------------------------------------------------------------------------------------------------------------- %
    [rows,cols,v] = find(A);
    if (size(rows,1) == 1)
        rows = rows';
